...
def skewness(x):
    # Fisher-Pearson ajustada (amostral)
    a = _to_1d_array(x)
    n = a.size
    if n < 3:
        return 0.0
...
def kurtosis_excess(x):
    # Excesso de curtose (Fisher) — Normal => 0
    a = _to_1d_array(x)
    n = a.size
    if n < 4:
        return 0.0
...
print(f"Curtose (excesso):     {curtose_excesso:,.4f}")

# Texto que vai dentro dos gráficos
box_text = (
    f"Assimetria: {assimetria:.4f}\n"
    f"Curtose (excesso): {curtose_excesso:.4f}"
)
box_style = dict(boxstyle="round,pad=0.3", fc="white", ec="gray", alpha=0.7)

# ---------------------------------------------------------
# Gráfico de colunas
# ---------------------------------------------------------
fig1, ax1 = plt.subplots(figsize=(14, 6))
ax1.bar(range(len(dados)), dados, color="skyblue", edgecolor="black")
...
# balão com assimetria e curtose
ax1.annotate(box_text, xy=(0.01, 0.98), xycoords="axes fraction",
             ha="left", va="top", bbox=box_style, fontsize=10)
fig1.tight_layout()
...
ax2.hist(dados, bins=10, edgecolor="black", color="salmon")
ax2.set_title("Distribuição dos Atos")
ax2.set_xlabel("Quantidade de atos")
ax2.set_ylabel("Frequência")
# balão com assimetria e curtose
ax2.annotate(box_text, xy=(0.01, 0.98), xycoords="axes fraction",
             ha="left", va="top", bbox=box_style, fontsize=10)
fig2.tight_layout()
fig2.savefig("histograma_distribuicao.png", dpi=300)
plt.show()

# Gráfico de linha (onda)
plt.figure(figsize=(14,6))
plt.plot(periodos, dados, marker="o", linestyle="-", color="blue")
plt.title("Atos praticados por período (gráfico de onda)")
plt.xlabel("Período")
plt.ylabel("Quantidade de atos")
plt.xticks(rotation=45)
plt.grid(True, linestyle="--", alpha=0.5)
plt.tight_layout()
plt.savefig("grafico_onda.png", dpi=300)
plt.show()
...
ax3.annotate(f"Assimetria (g₁) = {assimetria:.3f}\nLeve cauda à direita",
...
# (Assimetria Negativa, Simétrica e Positiva)
# ---------------------------------------------------------
def phi(x):
...
def skew_normal_pdf(x, alpha):
    # PDF da Normal Assimétrica: f(x) = 2 φ(x) Φ(αx)
...
for ax, cfg in zip(axes, curvas):
    ...
    ax.axhline(0, color="black", linewidth=2)          # “linha” na base como no exemplo
    ax.set_xticks([])
    ax.set_yticks([])
